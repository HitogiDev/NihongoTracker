...
**Logs** (`models/log.model.ts`): the central data model — one log per immersion session, `type` is one of `light-novel|reading|anime|vn|video|manga|audio|movie|tv show|other|game`. `light-novel` is the AniList-matchable type (a media discriminator exists for it); `reading` is a non-matchable free-form reading bucket (never shown on the Match Media screen / `getUntrackedLogs`) — both count toward the `reading` immersion category. Required fields (`episodes`/`pages`/`time`/`chars`) are conditionally required based on `type` via schema-level `required()`/`default()` functions — check this file before changing validation for any media type. Indexes are defined in-schema but only applied in `development`; production indexes are managed explicitly via `npm run migrate:indexes:prod` (see comment in the model).
...
**Dictionary popups** render Yomitan structured content through `@nihongotracker/yomitan-content`, a package shared with the desktop companion. It is maintained in that repository (`packages/yomitan-content`, where its tests are) and **vendored** here at `Frontend/vendor/yomitan-content` as a `file:./vendor/yomitan-content` dependency, so this repository builds on its own — a clone with no companion beside it, and the Docker image, whose context is this repository. Fix it upstream with a test, then re-copy `src/`; never edit the copy. `Frontend/vendor` is out of eslint's reach for that reason, and the Dockerfile copies it before `npm ci` because the dependency is unresolvable without it. The renderer never emits an `href` and never touches `innerHTML`: an internal cross-reference becomes a button that pushes onto `useLookupHistory`, anything else becomes plain text. An imported dictionary's `styles.css` goes through `scopeDictionaryCss` into a `<style>` inside the popup, never global — it came out of a zip the user chose. The package ships no strings; the back control and its label are `texthooker:dictionary.*` here.
