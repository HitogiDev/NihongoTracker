...
function GameLogs({ username, isActive = true }: GameLogsProps) {
  ...
  const {
    ...
    queryFn: () =>
      getUserLogsFn(username as string, { limit: 0, type: 'game' }),
  ...
  } = useSearch('game', shouldSearch ? searchQuery : '');
    ...
    <div className="w-full p-4">
                  ...
                  {searchResult.map((vn, i) => (
                    <div
                      ...
                      className={`flex gap-3 p-3 rounded-lg hover:bg-base-300 cursor-pointer ${
                        selectedGame?.contentId === vn.contentId
                          ? 'bg-primary/10 border border-primary'
                          : ''
                      }`}
                      onClick={() => setSelectedGame(vn)}
                    >
                      <div className="w-12">
                        <label className="cursor-pointer flex items-center justify-center h-full">
                          <input
                            ...
                            name="game"
                            checked={selectedGame?.contentId === vn.contentId}
                            onChange={() => setSelectedGame(vn)}
                          />
                        </label>
                      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mt-6">
        ...
        <button
          ...
          disabled={isAssigning || !selectedGame || selectedLogs.length === 0}
